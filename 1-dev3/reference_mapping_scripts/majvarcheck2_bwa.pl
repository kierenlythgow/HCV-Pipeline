...
#print "DAN:majvarcount:$majvarcount\tmajvarcount_OLD:$majvarcount_OLD\n\n";
  if(($majvarcount+$majvarcount_OLD) > 0){
    if($mvpath =~ /(\S+sus)(\d+)\.fasta/){
      ...
      $mvnew = $1.$x.".fasta.mv";
      $y = $x + 1;
      $bwa_index = $1.$x.".fasta.index";
      $consensus_fasta = $1.$x.".fasta";
      $sam = $1.$x.".fasta.bwa.sam";
      $bam = $1.$x.".fasta.bwa.bam";
      $sorted = $1.$x.".fasta.bwa.sorted";
      $sorted_bam = $1.$x.".fasta.bwa.sorted.bam";
      $pileup = $1.$x.".fasta.bwa.sorted.pileup";
      ...
      system("bwa index $consensus_fasta");
      system("bwa mem -t 8 $consensus_fasta $fwdreads $revreads > $sam");
